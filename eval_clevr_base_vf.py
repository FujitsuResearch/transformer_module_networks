...
def main():
    ...
    if not args.vf:
        from datasets.clevr_dataset import CLEVRDataset
        features_path_val = path_cfgs.path_dict_corpus_val['val_obj']
        annotation_path_val = path_cfgs.path_dict_annotation_val['val']
        ...
        if args.vf == 'region':
            ...
            print("select [region, vt]")
            exit()
    
        from models.base_model_vf import BaseTransformerWithExtractor, BaseTransformer
        from datasets.clevr_dataset_raw import CLEVRDataset

        transformer = BaseTransformer(config)
        model = BaseTransformerWithExtractor(config, transformer, extractor)
    ...
    for step, batch in enumerate(validation_data_loader):
        ...
        if not args.vf:
            features, spatials, image_mask, question, segment_ids, input_mask, co_attention_mask, answer_id, question_id = (batch)
            vil_prediction = \
                model(question, features, spatials, segment_ids, input_mask, image_mask, co_attention_mask)
        else:
            img, regions, img_info, spatials, image_mask, question, segment_ids, input_mask, co_attention_mask, answer_id, question_id = (batch)
            vil_prediction = \
